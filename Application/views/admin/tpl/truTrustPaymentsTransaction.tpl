...
[{foreach from=$labelGroupings key=groupingKey item=labelGrouping }]
    ...
    <table cellspacing="0" cellpadding="0" border="0" width="50%" id='TrustPayments-grouping-[{$groupingKey}]'>
        <tr>
...
[{if $oView->canVoid($oxid) }]
    <form name="truvoid" id="truvoid" method="POST" action="[{$oViewConf->getSelfLink()}]">
        <input type="hidden" name="fnc" value="void">
        <input type="hidden" name="cl" value="tru_trustPayments_transaction">
        <input type="hidden" name="cur" value="[{ $oCurr->id }]">
        <input type="hidden" name="oxid" value="[{ $oxid }]">
...
[{if $oView->canComplete($oxid) }]
    <form name="trucomplete" id="trucomplete" method="POST" action="[{$oViewConf->getSelfLink()}]">
...
[{if $oView->canRefund($oxid) }]
    <form name="trurefund" id="trurefund" method="POST" action="[{$oViewConf->getSelfLink()}]">
        <input type="hidden" name="cl" value="tru_trustPayments_refundjob">
        <input type="hidden" name="oxid" value="[{ $oxid }]">
