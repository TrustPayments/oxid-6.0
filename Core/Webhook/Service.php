...
use TrustPayments\Sdk\Model\WebhookListener;
...
class Service
{
    ...
    public function uninstall($spaceId, $url)
    {
        if ($spaceId !== null && !empty($url)) {
            $webhookUrl = $this->getWebhookUrl($spaceId, $url);
            if ($webhookUrl == null) {
                TrustPaymentsModule::log(Logger::ERROR, "Attempted to uninstall webhook with URL $url, but was not found.");
                return;
            }
    ...
    protected function getWebhookListenerService()
    {
        if ($this->webhookListenerService == null) {
            $this->webhookListenerService = new WebhookListenerService(TrustPaymentsModule::instance()->getApiClient());
        }
    ...
    protected function getWebhookUrlService()
    {
        if ($this->webhookUrlService == null) {
            $this->webhookUrlService = new WebhookUrlService(TrustPaymentsModule::instance()->getApiClient());
        }
