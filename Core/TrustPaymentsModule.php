...
use Tru\TrustPayments\Core\Provider\Language as LanguageProvider;
use Tru\TrustPayments\Application\Model\Transaction;

/**
 * Class TrustPaymentsModule
 * Handles module setup, provides additional tools and module related helpers.
 *
 * @codeCoverageIgnore
 */
class TrustPaymentsModule extends \OxidEsales\Eshop\Core\Module\Module
{
    const FALLBACK_LANGUAGE = 'en-US';
    const PAYMENT_PREFIX = 'oxidtru';
    /**
     * @var Logger
     */
    private $logger = null;
    ...
    {
        $sModuleId = 'truTrustPayments';

        $this->setModuleData(array(
            'id' => $sModuleId,
            'title' => 'TRU TrustPayments',
            'description' => 'TRU TrustPayments Module'
        ));
    ...
    public static function isTrustPaymentsPayment($paymentId)
    {
        return substr($paymentId, 0, strlen(self::PAYMENT_PREFIX)) === self::PAYMENT_PREFIX;
    }

    public static function extractTrustPaymentsId($paymentId)
    {
    ...
    {
        return self::getControllerUrl('tru_trustPayments_Webhook', 'notify');
    }
    ...
    public static function isAuthorizedState($state)
    {
        return in_array($state,
            [
                'TRUSTPAYMENTS_' . TransactionState::AUTHORIZED,
                'TRUSTPAYMENTS_' . TransactionState::FULFILL,
                'TRUSTPAYMENTS_' . TransactionState::COMPLETED
            ]);
    }
    ...
    public function translate($sCode, $blUseModulePrefix = true, $args = array())
    {
        if ($blUseModulePrefix) {
            $sCode = 'tru_trustPayments_' . $sCode;
        }
     ...
     * @return string
     */
    public function TrustPaymentsTranslate(array $map)
    {
    ...
    {
    	return \OxidEsales\Eshop\Core\Registry::getConfig()->getModulesDir() . 'tru/TrustPayments/';
    }
    ...
    private function initLogger()
    {
        try {
        	if(class_exists(Logger::class)) {
	            $this->logger = new Logger('TrustPayments',
	                array(
    ...
    public static function rollback()
    {
        try {
            ...
            TrustPaymentsModule::log(Logger::ERROR, "UNABLE TO ROLLBACK: {$e->getMessage()} - {$e->getTraceAsString()}.");
        }
    }
}